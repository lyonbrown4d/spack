...
func TestHealthcheckCommandSucceedsForHealthyEndpoint(t *testing.T) {
	err := healthcheckcmd.RunForTest("http://spack.test/livez", newHealthcheckClient(http.StatusNoContent))
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestHealthcheckCommandFailsForUnhealthyEndpoint(t *testing.T) {
	err := healthcheckcmd.RunForTest("http://spack.test/livez", newHealthcheckClient(http.StatusServiceUnavailable))
	if err == nil {
