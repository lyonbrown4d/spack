...
func TestValidateConfiguredAssetsRootAcceptsDirectoryAndBundle(t *testing.T) {
	root := t.TempDir()
	if err := configcmd.ValidateConfiguredAssetsRootForTest(root); err != nil {
		t.Fatal(err)
	}
	...
	if err := configcmd.ValidateConfiguredAssetsRootForTest(bundlePath); err != nil {
		t.Fatal(err)
	}
}
...
func TestValidateConfiguredAssetsRootRejectsBrokenBundle(t *testing.T) {
	...
	err := configcmd.ValidateConfiguredAssetsRootForTest(bundlePath)
	if err == nil {
